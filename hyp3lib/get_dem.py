...
import configparser
from hyp3lib.asf_geometry import raster_meta, geotiff2data, data2geotiff, geometry2shape
...
def get_dem_list(config_file):
    ...
    for name in dems:
        ...
        if tile_file.startswith('http'):
            sample_tile_dir = "DEM-sample-{0}".format(name)
            os.mkdir(sample_tile_dir)
            download_file(tile_file, directory=sample_tile_dir)
            tile_file_new = os.path.join(sample_tile_dir, tile_file.rsplit('/', 1)[-1])
            tile_file = tile_file_new
...
def change_reference(x_min, x_max, y_min, y_max, posting, inPixelRef,
    ...
    '''
    Pixel reference 'Area' refers to the center of the pixel.
    Pixel reference 'Point' refers to the upper left corner of the pixel.
    '''
...
def scale_posting(bestDEM, posting, out_proj):
    ...
    logging.info("Scale posting input values: {0} | {1} | {2} ".format(bestDEM, posting, out_proj))
    ...
    if out_proj == 'dem':
        ...
        if out_proj == 'utm' or out_proj == 'polar':
            ...
            if bestDEM['name'] == 'TDT':
                demPosting = 0.00027777777778 * scale
            else:
                demPosting = bestDEM['posting'] * scale

        logging.info("Scale : {0} | option 1 : {1} | option 2 : {2}".format(scale, (demPosting / posting), (posting / demPosting)))
    ...
    logging.info('res value returned: {0} | demPosting used: {1}'.format(res, demPosting))

    return res


def get_dem(x_min, y_min, x_max, y_max, outfile, post=0.0, processes=4,
    dem_name=None, leave=False, config_file=None, snap_to_grid=False,
    pixel_reference='Point', dem_type='utm', out_format='GeoTIFF'):
    ...
    if scaleOnly == False:
        ...
        if scale > 0:
            ...
            gdal.Translate(scaled_file, VRT, heightPct=scale, widthPct=scale,
                format='GTiff')
            logging.info("Mapping DEM mosaic into output space after Resampling DEM...")
