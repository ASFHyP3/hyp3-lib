...
def prepare_dem_geotiff(
    ...
    The DEM mosaic is assembled from the Copernicus GLO-30 Public DEM. The output GeoTIFF covers the extent of the input
    geometry.
    ...
    Args:
        ...
        geometry: Geometry in EPSG:4326 (lon/lat) projection for which to prepare a DEM mosaic
          MULTIPOLYGON geometries are assumed to cross the antimeridian with one polygon in each hemisphere.
        ...
        height_above_ellipsoid:
          ...
          If True, the output GeoTIFF will contain height above ellipsoid values.
    """
    ...
    with GDALConfigManager(GDAL_DISABLE_READDIR_ON_OPEN='EMPTY_DIR'):
        with TemporaryDirectory() as temp_dir:
            ...
            dem_file_paths = _get_dem_file_paths(geometry)
